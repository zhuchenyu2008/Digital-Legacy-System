...
> 关联文档：[系统架构设计](./02-system-architecture.md) · [数据库设计](./03-database-design.md) · [API 接口设计](./04-api-design.md) · [安全与隐私设计](./05-security-privacy.md) · [页面设计与交互规格](./06-page-specifications.md) · [工程与运维设计](./07-implementation-and-operations.md) · [测试与验收计划](./08-test-and-acceptance-plan.md)
...
4. 在发布前，使仅取得数据库或文件存储后端的攻击者无法解密 ZIP。
...
- **PRD-FILE-003**：上传内容在浏览器端分块加密后进入文件存储后端；默认使用受保护的本地持久卷，S3 兼容存储可选，私有存储只保存密文。
...
- **PRD-RECOVERY-001**：登录页点击忘记主密码后，系统只向已配置的主邮箱发送一次性启动链接，不立即公开状态或通知联系人；备用邮箱不自动接收同一恢复令牌或验证码。
...
- **PRD-RECOVERY-005**：达到门限后，系统只向主邮箱发送设置新主密码的单次链接和一次性验证码；验证码为 CSPRNG 生成的 8 位数字，有效期 10 分钟，最多连续尝试 5 次，不能放入 URL 或日志。联系人不能设置或获知新主密码。
- **PRD-RECOVERY-006**：管理员必须同时提交未消费的主邮箱链接令牌和验证码，才能建立一次性密钥重包装会话；设置新主密码后重新包装管理员密钥，销毁恢复过程中的临时密钥，记录当天签到并返回 `ARMED`。
...
- **PRD-RECOVERY-010**：验证码只证明对主邮箱的短期控制，不视为独立 MFA；管理员仍须在上线确认中知悉无 MFA 的账号风险。
...
> 超过半数有效联系人确认后，只有 {owner_name} 的主邮箱链接和验证码能够设置新密码；备用邮箱不会收到同一验证码。
...
8. **文件保密**：未达到任何门限时，仅取得数据库、文件存储后端和应用配置但没有任何用户密码，不得解密 ZIP。
...
当前设计按以下风险假设建立，产品不得通过文案掩盖；正式上线前必须由管理员逐项确认：

- 紧急联系人可能误判、合谋、泄露密码或遭账号攻击。
- 达到门限后，服务器在第二阶段必须临时具备解密能力，不能再称为严格零知识。
- 主密码同时承担登录、签到、终止和管理员密钥保护职责，泄露影响重大。
- 不启用双因素认证会降低账号保护能力。
- 主邮箱验证码是邮箱持有证明和恢复流程的第二个步骤，不等同于独立 MFA；邮箱账户本身被接管时仍需依赖联系人门限保护。
- SMTP 接收成功不等于邮件必然被阅读。
- 公开内容无法真正永久控制、撤回或阻止第三方传播。
- 不做备份意味着服务器、数据库或文件存储后端损坏后可能永久丢失。
- 系统不能保证遗嘱文件符合法定形式或最终被法院采纳。

## 11. 产品决策确认规则

本文件中的业务流程和安全边界是实现基线，但部署地区、备份、MFA、正式域名、SMTP、文件存储类型、外部监控和法律审核仍需由产品所有者确认。默认文件卷不要求额外供应商；S3 兼容存储仅在部署者显式选择时配置。

在这些决策确认前：

1. 不得把境外部署、无备份、无 MFA 或不可撤回发布写成已经完成的上线条件；
2. 不得使用真实个人信息、真实遗书或正式邮箱进行开发和测试；
3. 不得因为页面或接口已设计，就默认产品所有者已经接受相应风险；
4. 任何改变门限、状态机、公开范围、联系人隐私或发布不可逆性的决定，都必须同时更新数据库、API、安全、页面和测试文档。

产品所有者确认后，必须记录决策编号、决定内容、北京时间、影响文档和对应验收证据。
