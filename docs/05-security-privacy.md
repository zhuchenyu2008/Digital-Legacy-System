...
> 关联文档：[产品需求](./01-product-requirements.md) · [系统架构](./02-system-architecture.md) · [数据库设计](./03-database-design.md) · [API 设计](./04-api-design.md) · [页面规格](./06-page-specifications.md) · [工程与运维设计](./07-implementation-and-operations.md) · [测试与验收计划](./08-test-and-acceptance-plan.md)

## 1. 安全结论

本系统采用“发布前门限保护、门限后受控自动解密”的模型：

- 正常状态和首次确认未达到门限时，数据库、文件存储后端和部署秘密的离线泄露不足以直接解密遗书 ZIP；仍需管理员主密码或足够联系人实际解包并提交分片。
...
- 网站端加密不能抵御已经控制线上 Web 服务器并篡改 JavaScript 的攻击者；此类攻击者可以等待用户下次输入密码。浏览器端加密主要抵御数据库、文件存储、磁盘快照和静态备份类泄露。
...
| 私有文件卷或 S3 私有桶 | 不信任其保密性 | 只存认证密文；不位于 Web Root |
| 公开文件卷或 S3 公开桶 | 公开 | 发布后无保密目标；通过下载处理器读取 |
...
- 密码先执行 Unicode NFC，再编码为 UTF-8；不 `trim`、不大小写折叠，并同时限制不超过 512 个 UTF-8 字节。浏览器和服务端必须使用同一规范化函数和版本。
...
当前设计暂按不启用 MFA 的场景采用较强密码基线。若 `DEC-004` 改为启用 MFA，仍不得降低密码和密钥保护基线。OWASP 当前建议无 MFA 时少于 15 字符应视为弱密码，并建议最大长度至少允许 64 字符、允许 Unicode/空格、阻止常见泄露密码。[OWASP Authentication Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html)

### 4.2 认证哈希

使用 Argon2id，保存标准 PHC 字符串。认证输入先使用版本化 `PASSWORD_PEPPER_v` 做 HMAC 预处理；pepper 版本不写入 PHC，而单独保存。初始参数：
...
salt        每个凭据独立随机 16 字节
output      32 字节
 pepper      `HMAC-SHA-256(PASSWORD_PEPPER_v, length-prefix("dls/password-auth/v1", UTF8(NFC(password))))`
```
...
认证哈希与密钥派生必须使用不同盐、用途标签和输出，禁止把 `password_phc` 当作加密密钥。pepper 轮换时旧版本短期双读，成功认证后使用新版本重哈希；不能删除旧 pepper 后再指望从 PHC 反推出新哈希。

### 4.3 密钥派生

管理员 `OWNER_KEK` 和联系人 `CONTACT_KEK` 在浏览器中使用 Argon2id 派生，参数随包装保存并可版本化；客户端密钥派生不使用服务端 pepper。每个包装使用独立随机 24 字节 nonce 的 XChaCha20-Poly1305。关联 AAD 必须使用长度前缀编码并包含：

```text
protocolVersion | purpose | actorId | vaultId/contactId | kdfParamsHash
```

另外还必须包含对象/代次 ID、算法版本和 `vkCommitment` 或 `packageId` 绑定值。改密流程先用旧密钥认证解密，再用新盐和新派生密钥重加密，并提交服务端可验证的包装证明。任何认证失败不得输出“密码正确但密钥损坏”等可区分错误。
...
| 主邮箱恢复验证码 | 每挑战最多 5 次；每 IP 每小时 10 次；挑战 10 分钟有效 |
...
- 所有包装明确记录协议版本、算法、AAD 摘要和密钥版本；nonce 不得重复使用。
- 私有文件存储只保存密文、非秘密流头和必要元数据；默认文件卷与可选 S3 适配器执行同一安全契约。
- 上传完成时由服务端读取对象并重新计算真实大小和密文 SHA-256；客户端摘要和 manifest 只作为绑定比对值。
...
- 临时死亡分片使用 `RELEASE_STAGE_KEK`，恢复分片使用 `RECOVERY_STAGE_KEK`；达门限、取消或过期后立即删除。
- 分片代次必须包含经评审的可公开验证秘密共享证明，服务端在激活代次和接收流程分片时验证其与当前 `vkCommitment`、代次、联系人和用途一致。
...
```text
 RELEASE_STAGE_KEK  包装死亡发布阶段的临时 VK，仅 Worker 可读
 RECOVERY_STAGE_KEK 包装密码恢复阶段的临时 VK，仅 API 可读
PII_KEK         加密姓名、邮箱、IP、User-Agent
PII_INDEX_KEY   生成姓名/邮箱盲索引
TOKEN_HMAC_KEY  单次令牌和会话令牌摘要
AUDIT_CHAIN_KEY 私有审计链 HMAC
 PASSWORD_PEPPER_v 认证哈希纵深保护，按版本轮换
```

要求：

- 通过 Docker Secret、只读 root-owned 文件或云秘密管理服务注入，不直接写普通环境变量。
- 文件权限仅对应 API 或 Worker UID 可读；容器以非 root 运行。API 不得读取 `RELEASE_STAGE_KEK`，Worker 不得读取 `RECOVERY_STAGE_KEK`。
- 每项秘密独立生成至少 256 位随机值，不能从主密码或同一根字符串派生。
- 数据库记录密钥版本，不记录密钥本身。
- 轮换 `PII_KEK` 时后台逐条重加密；轮换索引键需要双写和重建索引。
- 轮换任一 stage key 前必须处理对应活动会话；旧版本保留到这些会话结束，并按 `stage_key_purpose`/版本恢复。
- 当前无备份工作假设下，密钥文件丢失会永久破坏对应数据；这是上线前必须由管理员确认的风险。

### 5.5 密码学信任边界

“没有密码无法解密”具体定义为：在正常状态或首次确认尚未达到门限时，攻击者只得到数据库、私有对象桶和部署 secret 文件，不能在可行成本内恢复 `VK`；该保证依赖主密码具有足够熵，不能表述为数学上不可破解。以下情况不在该保证内：
...
- 控制管理员主邮箱并控制过半联系人，可接管管理员账号并读取遗书，低于死亡发布门限；备用邮箱不自动接收同一验证码。
- 只有主邮箱链接、验证码和联系人门限同时满足才能设置新密码；只控制邮箱或只控制联系人均不能恢复。
- 服务端在恢复门限达到后短期持有 `VK`，其运行环境被控时可以读取遗书。
- 恢复开始必须由管理员主邮箱单次链接触发，公开按钮不能直接进入联系人投票。
- 达到门限后，主邮箱收到单次重置链接和 8 位验证码；验证码 10 分钟有效、最多 5 次，数据库只存 HMAC，不能放入 URL 或日志。
- 恢复链接 7 天过期；成功或过期后销毁所有分片、令牌和临时密钥。
- 恢复流程不暂停死亡截止，防止攻击者借恢复流程无限阻止发布。
- 达到门限并通过主邮箱验证码后，管理员浏览器生成一次性 X25519 临时密钥对；服务器仅把 `VK` sealed-box 加密给该临时公钥。浏览器用新主密码重包 `VK` 后立即销毁临时私钥和明文 `VK`，重包装会话最长 15 分钟且只能使用一次。
...
- 会话 HMAC 和单次令牌都记录 `TOKEN_HMAC_KEY` 版本，轮换期间只允许受限双版本校验，完成后吊销旧版本。
...
初始化、邀请消费、恢复启动、恢复验证码和重包装等无会话入口必须额外要求严格 `Origin`、`Sec-Fetch-Site: same-origin`、JSON 请求体和 `Cache-Control: no-store`；它们不得因为携带一次性令牌就放宽跨站校验。
...
- 普通提醒和非恢复通知在主邮箱 SMTP 明确失败后立即尝试备用邮箱；主密码恢复启动链接和验证码只发主邮箱，不得自动回退到备用邮箱。“SMTP 接受”不代表收件箱实际可见。
...
- `VK`、`DEK`、`RELEASE_STAGE_KEK`、`RECOVERY_STAGE_KEK`、联系人私钥或 Shamir 分片；
- SMTP 密码、数据库连接串；
...
| ZIP 密文 | 遗产保管 | 文件存储后端；授权加密流程 | 当前版本期间 |
...
本设计暂以服务器位于中国大陆以外作为工作假设，但正式部署地区、备案和跨境合规方案仍待产品所有者确认。部署者必须确认实际服务器所在地、管理员和联系人所在地、SMTP 供应商，以及选择 S3 时的存储供应商和跨境提供个人信息的适用要求。本文件不构成法律意见。

## 13. 基础设施加固

- Linux 主机仅开放 22（限制来源）、80、443；SSH 禁止密码登录和 root 直登。
- 容器使用非 root UID、只读根文件系统、删除非必要 Linux capabilities、设置 CPU/内存/PID 限额。
- PostgreSQL 使用最小权限角色；默认文件卷按容器 UID 隔离。选择 S3 时，存储凭据同样最小授权且 API 与 Worker 账号分离。
...
- 使用 SMTP 沙箱和默认文件卷测试重启恢复、延迟与失败路径；可选 S3 适配器使用 MinIO 执行同一测试。
...
| `RELEASE_STAGE_KEK` 泄露 | 立即停止发布 Worker，轮换发布 stage key，检查活动第二阶段和异常解密审计 |
| `RECOVERY_STAGE_KEK` 泄露 | 立即停止恢复接口，轮换恢复 stage key，吊销验证码/令牌/重包装会话并检查审计 |
| 数据库泄露 | 轮换 pepper、令牌/HMAC/PII 密钥，按版本双读后强制改密并重新分片 |
...
4. 任意少于死亡/恢复门限的分片组合均不能重建 `VK`，且公开验证证明绑定正确 `vkCommitment`。
5. 初始化、联系人注册、普通改密和重分片的包装证明错误时均被拒绝。
6. 主邮箱验证码投递、过期、5 次锁定、链接/验证码双消费测试通过。
7. 加密 ZIP 篡改、截断和错序均被拒绝，服务端真实摘要校验通过。
8. ZIP 根目录 `will.md` 验证、路径穿越和 ZIP bomb 防护通过。
9. 管理员取消与到期发布的并发测试只产生一个终态。
10. 第二阶段后联系人所有终止接口均被拒绝，且知情书包含该条款。
11. 最终邮件只包含公开链接，不附 ZIP；SMTP 失败不阻塞发布。
12. 公开 API 和公开审计不含联系人身份及私有日志字段。
13. 测试模式收件白名单和对象前缀隔离通过。
14. 发布表、公开审计和公开对象不存在普通应用权限下的删除路径。
15. 管理员再次确认无 MFA、无备份、发布不可撤回和法律效力不保证的风险。
...
文档中的“必须”控制只有在以下证据归档后才算完成：

| 控制 | 必须证据 | 当前状态 |
|---|---|---|
| TLS、HSTS、Cookie、CSP、CSRF | 自动化 HTTP 安全测试和配置快照 | 未执行 |
| Argon2id、密码日志保护 | 参数基准、弱密码测试、日志扫描 | 未执行 |
| Shamir、secretstream、密钥包装 | 固定测试向量、少于/达到门限测试、篡改测试 | 未执行 |
| VK/私钥/分片绑定证明 | 初始化、注册、改密、重分片的错误绑定测试 | 未执行 |
| 主邮箱恢复验证码 | 主邮箱投递、10 分钟过期、5 次锁定、链接/验证码双重消费测试 | 未执行 |
| ZIP 安全 | 恶意 ZIP 样本、路径穿越、ZIP bomb、符号链接测试 | 未执行 |
| 并发状态机 | 数据库并发测试和唯一终态证据 | 未执行 |
| 发布不可撤回 | 普通角色权限、API、触发器和对象权限测试 | 未执行 |
| 测试隔离 | 收件人、数据库、对象前缀、公开地址隔离测试 | 未执行 |
| 隐私与法律 | 知情书、隐私政策和律师评审记录 | 未完成 |

“设计上要求”与“已被测试证明”必须在发布清单中分开记录。安全文档本身不能替代密码学专家评审、渗透测试或法律审核。
