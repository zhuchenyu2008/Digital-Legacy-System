...
describe("CSRF, Origin, and CORS boundaries", () => {
  it("allows only the configured public origin in production", () => {
    const production = {
      ...
      FIELD_KEYRING: JSON.stringify({
        version: 1,
        activeVersion: 1,
        lookupKey: Buffer.alloc(32, 5).toString("base64"),
        lookupKeys: { 1: Buffer.alloc(32, 5).toString("base64") },
        keys: { 1: Buffer.alloc(32, 6).toString("base64") },
      }),
