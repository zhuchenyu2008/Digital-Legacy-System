...
[[ -n "$BACKUP_ROOT" && -n "$DATA_ROOT" && -n "$SECRETS_ROOT" && -n "$ENV_FILE" && -n "$STATUS_FILE" && -n "$ENCRYPTION_KEY_FILE" ]] || {
  echo "backup, data, secrets, env, status, and encryption key paths are required" >&2
  exit 2
}
...
[[ "$BACKUP_ROOT" != / && "$DATA_ROOT" != / && "$SECRETS_ROOT" != / ]] || { echo "automation paths must not be /" >&2; exit 1; }
...
node "$ROOT/ops/scripts/write-operation-status.mjs" --file "$STATUS_FILE" --kind backup --status ok --started-at "$STARTED_AT" --artifact "$FINAL"
COMPLETED=1
