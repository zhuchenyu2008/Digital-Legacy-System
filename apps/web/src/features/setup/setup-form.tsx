...
export function SetupForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (checked.normalized !== form.confirmation.normalize("NFC")) {
    ...
    try {
      ...
      const response = await apiRequest<{ data: { session?: { csrfToken: string } } }>(
        "/setup/owner",
        {
          ...
          body: JSON.stringify({
            ...
            ownerVaultEnvelope: cryptoResult.envelope,
          }),
        },
      );
      setForm({
      ...
      setMessage(`初始化失败，请检查输入后重试${requestId ? `。请求编号：${requestId}` : ""}`);
      setForm((current) => ({ ...current, password: "", confirmation: "", setupToken: "" }));
    } finally {
      setBusy(false);
    }
  }
  return (
    <form className="dls-form-stack" onSubmit={submit}>
      ...
      <Field
        ...
        hint="至少 12 个字符，最多 512 个 UTF-8 字节。"
        id="new-password"
        label="主密码"
        name="password"
      ...
      />
      {message ? (
        <Toast tone={message.startsWith("正在") ? "info" : "error"}>{message}</Toast>
      ) : null}
