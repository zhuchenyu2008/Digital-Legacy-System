...
export function OwnerPasswordChange() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const material = await apiRequest<{
        data: { vaultId: string; ownerVaultEnvelope: Record<string, unknown> };
      }>("/owner/vault/material");
      const wrapped = await createCryptoWorkerClient().run<{ envelope: Record<string, unknown> }>(
        "rewrapOwnerVault",
        {
          oldPassword,
          newPassword: checked.normalized,
          envelope: material.data.ownerVaultEnvelope,
          vaultId: material.data.vaultId,
        },
      );
      await apiRequest("/auth/owner/password-change", {
        method: "POST",
        body: JSON.stringify({
          oldPassword,
          newPassword: checked.normalized,
          newOwnerVaultEnvelope: wrapped.envelope,
        }),
      });
      setOldPassword("");
      setNewPassword("");
      setConfirmation("");
      setMessage("主密码已修改，其他管理员会话已撤销。");
    } catch (error) {
      const requestId = requestIdFrom(error);
      setMessage(`修改失败${requestId ? `。请求编号：${requestId}` : ""}`);
    } finally {
      setBusy(false);
    }
  }
  return (
    <form className="dls-panel dls-form-stack" onSubmit={submit}>
      ...
      {message ? (
        <Toast tone={message.startsWith("主密码已") ? "success" : "error"}>{message}</Toast>
      ) : null}
    </form>
  );
}
